...
export function FallbackStation({ currentStation, className }: FallbackStationProps) {
  if (!currentStation.fallbackStationId) return null

  const fallback = stations.find((s) => s.id === currentStation.fallbackStationId)
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex items-center justify-between gap-2">
            <div className="min-w-0">
              <p className="truncate text-sm font-medium text-foreground">{fallback.name}</p>
              <p className="text-xs text-muted-foreground">{fallback.city}</p>
            </div>
